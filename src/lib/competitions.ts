...
export const normalizeCompetition = (competition: CompetitionApiResponse): Competition => ({
  _id: competition._id,
...
export const normalizeCompetitions = (competitions: CompetitionApiResponse[]): Competition[] =>
  competitions.map(normalizeCompetition);
...
export const buildCompetitionPayload = (data: Partial<Competition>) => {
  const payload = {
    competition_id: trimOrUndefined(data.competition_id),
