...
export async function exportCompetitions(
  ...
  const filename =
    options.filename ||
    `competitions_${new Date().toISOString().split("T")[0]}.${options.format}`;
...
export async function exportVenues(options: ExportOptions = { format: "csv" }) {
  ...
  const filename =
    options.filename ||
    `venues_${new Date().toISOString().split("T")[0]}.${options.format}`;
...
export async function exportReferees(
  options: ExportOptions = { format: "csv" },
) {
  ...
  const filename =
    options.filename ||
    `referees_${new Date().toISOString().split("T")[0]}.${options.format}`;
...
export async function exportPlayers(
  ...
  const filename =
    options.filename ||
    `players_${new Date().toISOString().split("T")[0]}.${options.format}`;
...
export function downloadPlayerWithTeamTemplate(format: ExportFormat = "csv") {
  const template = [
    {
      name: "Example Player",
      position: "ST",
      ...
      player_height: 180,
      player_weight: 75,
      team_name: "Existing Team FC",
      jersey_number: 9,
    },
  ];

  const filename = `player_with_team_template.${format}`;
...
export async function exportTeams(options: ExportOptions = { format: "csv" }) {
  ...
  const filename =
    options.filename ||
    `teams_${new Date().toISOString().split("T")[0]}.${options.format}`;
...
export async function exportMatches(
  ...
  const filename =
    options.filename ||
    `matches_${new Date().toISOString().split("T")[0]}.${options.format}`;
    ...
    const jsonData = {
      ...
      teams: [
        ...
        },
        {
          ...
          founded_year: 1970,
          stadium: "Camp Nou",
          managers: [{ name: "Jonatan Giráldez" }],
        },
      ],
    };
