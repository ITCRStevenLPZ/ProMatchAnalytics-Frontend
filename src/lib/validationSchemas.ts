...
export const playerSchema = Joi.object({
  ...
  country_name: Joi.string().min(2).max(100).required().messages({
  ...
  player_height: Joi.number().min(100).max(250).optional().messages({
  ...
  player_weight: Joi.number().min(30).max(150).optional().messages({
...
export const teamSchema = Joi.object({
  ...
  short_name: Joi.string().min(2).max(10).required().messages({
...
export const competitionSchema = Joi.object({
  ...
  short_name: Joi.string().min(2).max(20).optional().allow("").messages({
    "string.min": "Short name must be at least 2 characters",
