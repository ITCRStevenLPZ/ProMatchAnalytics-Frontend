...
export default function ConflictDiffViewer({ conflict }: ConflictDiffViewerProps) {
    ...
    <div className="bg-white border rounded-lg p-4">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="text-sm text-gray-500">
          {fieldPaths.filter((fp) => fields[fp].is_significant).length} significant differences
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`border rounded-md p-3 ${
                isSignificant ? 'border-yellow-300 bg-yellow-50' : 'border-gray-200'
              }`}
            >
              <div className="flex items-center mb-2">
                <span className="font-medium text-gray-700">{diff.field_path}</span>
                {isSignificant && (
