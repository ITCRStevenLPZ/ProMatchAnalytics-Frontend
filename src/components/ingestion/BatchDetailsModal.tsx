...
export function BatchDetailsModal({ batch, onClose, onOpenBatch }: BatchDetailsModalProps) {
  ...
  const stats = [
    ...
    { label: t('ingestion.duplicatesSkipped', 'Duplicates'), value: batch.duplicates_discarded },
      ...
      <div className="w-full max-w-3xl bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <div>
            ...
            <h2 className="text-xl font-bold text-gray-900">{batch.batch_name}</h2>
            <p className="text-sm text-gray-500 font-mono">{batch.ingestion_id}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 space-y-4">
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-blue-900">
                {t('ingestion.targetModel')} · <span className="capitalize">{batch.target_model}</span>
              </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
            <div>
              <p className="font-semibold text-gray-900 mb-1">{t('ingestion.status')}</p>
              <p className="capitalize">{batch.status.replace('_', ' ')}</p>
            </div>
            <div>
              <p className="font-semibold text-gray-900 mb-1">{t('ingestion.expiresAt')}</p>
              {batch.expires_at ? (
                <p>
                  {new Date(batch.expires_at).toLocaleString()} ·{' '}
                  {t('ingestion.daysRemaining', { count: Math.max(0, Math.ceil((new Date(batch.expires_at).getTime() - Date.now()) / (1000 * 60 * 60 * 24))) })}
                </p>
              ) : (
                <p className="text-gray-400">{t('ingestion.noExpiration', 'No expiration')}</p>
              )}
            </div>
          </div>
        </div>
