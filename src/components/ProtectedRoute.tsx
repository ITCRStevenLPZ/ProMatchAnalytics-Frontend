    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-red-600 mb-4">{t('errors.forbidden')}</h1>
          <p className="text-gray-600">{t('errors.noPermission')}</p>
          <p className="text-sm text-gray-500 mt-2">{t('errors.allowedRoles')}: {allowedRoles.join(', ')}</p>
          <p className="text-sm text-gray-500">{t('errors.yourRole')}: {user.role || t('errors.unknown')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-red-600 mb-4">{t('errors.forbidden')}</h1>
          <p className="text-gray-600">{t('errors.noPermission')}</p>
          <p className="text-sm text-gray-500 mt-2">{t('errors.requiredRole')}: {requiredRole}</p>
          <p className="text-sm text-gray-500">{t('errors.yourRole')}: {user.role || t('errors.unknown')}</p>
        </div>
      </div>
    );
