...
import { X, AlertTriangle, Users, ArrowRight, Clock, ShieldAlert } from 'lucide-react';
...
interface SubstitutionFlowProps {
  ...
  period: number;
  globalClock: string;
  onSubmit: (playerOffId: string, playerOnId: string, isConcussion: boolean) => void;
  onCancel: () => void;
}
...
export default function SubstitutionFlow({
  ...
  const [step, setStep] = useState<'select-off' | 'select-on' | 'confirm'>('select-off');
  ...
  // Validate substitution when both players are selected
  useEffect(() => {
    if (playerOff && playerOn && step === 'confirm') {
      const validateSubstitution = async () => {
        setIsValidating(true);
        try {
          const token = localStorage.getItem('authToken');
          const response = await fetch(
            `http://localhost:8000/api/v1/logger/matches/${matchId}/validate-substitution`,
            {
              method: 'POST',
              headers: {
          ...
          setValidation({
            is_valid: false,
            error_message: t('substitution.validationError', 'Failed to validate substitution'),
  ...
  }, [playerOff, playerOn, step, isConcussion, matchId, team.id, period, t]);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" data-testid="substitution-modal">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-blue-700 text-white p-6 rounded-t-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Users size={28} />
              <div>
                <h2 className="text-2xl font-bold" data-testid="substitution-heading">
                  {t('substitution.title', 'Substitution')}
                </h2>
                <p className="text-blue-100 text-sm">
                  {team.name} • {globalClock} • {t(`period.${period}`, `Period ${period}`)}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
              <div className="bg-blue-800 bg-opacity-50 rounded p-3">
                ...
                <div className="text-2xl font-bold">
                  {teamStatus.remaining_substitutions}/{teamStatus.max_substitutions}
                </div>
              </div>
              <div className="bg-blue-800 bg-opacity-50 rounded p-3">
                <div className="text-blue-200 text-xs mb-1">
        ...
        <div className="flex items-center justify-center gap-4 p-6 bg-gray-50 dark:bg-gray-900">
          <div className={`flex items-center gap-2 ${step === 'select-off' ? 'text-blue-600 font-semibold' : 'text-gray-400'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'select-off' ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}>
              1
            </div>
            <span>{t('substitution.selectPlayerOff', 'Player Off')}</span>
          </div>
          <ArrowRight size={20} className="text-gray-400" />
          <div className={`flex items-center gap-2 ${step === 'select-on' ? 'text-blue-600 font-semibold' : 'text-gray-400'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${step === 'select-on' ? 'bg-blue-600 text-white' : 'bg-gray-300'}`}>
              2
            </div>
        ...
        {/* Content */}
        <div className="p-6">
          {/* Step 1: Select Player Off */}
          {step === 'select-off' && (
            <div data-testid="substitution-step-off">
              <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
                {t('substitution.whoIsLeavingField', 'Who is leaving the field?')}
              </h3>
          ...
          {step === 'select-on' && playerOff && (
            <div data-testid="substitution-step-on">
              ...
              <h3 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white">
                {t('substitution.whoIsEnteringField', 'Who is entering the field?')}
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          {step === 'confirm' && playerOff && playerOn && (
            <div data-testid="substitution-step-confirm">
              <div className="space-y-4 mb-6">
                {/* Players Summary */}
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-red-50 dark:bg-red-900 border-2 border-red-300 dark:border-red-700 rounded-lg">
                    <div className="text-sm text-red-700 dark:text-red-200 mb-2 font-semibold">
                  ...
                  <div className="p-4 bg-green-50 dark:bg-green-900 border-2 border-green-300 dark:border-green-700 rounded-lg">
                    <div className="text-sm text-green-700 dark:text-green-200 mb-2 font-semibold">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <div className="font-semibold text-gray-900 dark:text-white">
                          {t('substitution.concussionSubstitution', 'Concussion Substitution')}
                        </div>
                        <div className="text-sm text-gray-600 dark:text-gray-400">
                          {t('substitution.concussionNote', 'Does not count toward maximum. Opponent receives additional substitution.')}
                        </div>
                      </div>
                    </div>
                  </label>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="font-semibold text-blue-800 dark:text-blue-200 mb-1">
                            {t('substitution.newWindow', 'New Substitution Window')}
                          </div>
                          <div className="text-blue-700 dark:text-blue-300">
                            {t('substitution.newWindowNote', 'This substitution will open a new substitution window.')}
                          </div>
                        </div>
                      </div>
                    )}
          ...
          <div className="flex gap-3 justify-end">
            ...
            {step === 'confirm' && (
              <button
                ...
                disabled={isValidating || !validation?.is_valid}
