...
export function MatchAnalytics({ match, events, effectiveTime, t }: MatchAnalyticsProps) {
  ...
  const analytics = useMemo(() => {
    ...
    const timeline: EventTimeline[] = Array.from(timelineMap.entries())
      ...
      .filter((_, idx, arr) => idx === 0 || idx === arr.length - 1 || timelineMap.get((_ as any).minute)!.home + timelineMap.get((_ as any).minute)!.away > 0);
    ...
    events.forEach((event) => {
      if (event.player_id) {
        if (!playerMap.has(event.player_id)) {
          const team = event.team_id === homeTeamId ? match.home_team : match.away_team;
          const player = team.players.find((p) => p.id === event.player_id);
          playerMap.set(event.player_id, {
            player_id: event.player_id,
            player_name: player?.short_name || player?.full_name || `#${event.player_id}`,
            events: 0,
            passes: 0,
            shots: 0,
          });
    ...
    const totalPasses = getEventCount(homeEvents, 'Pass') + getEventCount(awayEvents, 'Pass');
    ...
    const avgEventsPerMinute = maxMinute > 0 ? (totalEvents / maxMinute).toFixed(1) : '0.0';
    const mostActiveMinute = Array.from(timelineMap.entries())
      .reduce((max, [min, counts]) => {
    ...
    return (
      <div className="flex items-center justify-center h-64 text-gray-400">
        ...
        <div className="text-lg">
          {t('analytics.noData', 'No data available yet. Start logging events!')}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" data-testid="analytics-panel">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2" data-testid="analytics-title">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={analytics.timeline}>
            ...
            <XAxis 
              dataKey="minute" 
              label={{ value: t('analytics.minutes', 'Minutes'), position: 'insideBottom', offset: -5 }}
              stroke="#6b7280"
            />
            <YAxis 
              label={{ value: t('analytics.events', 'Events'), angle: -90, position: 'insideLeft' }}
              stroke="#6b7280"
            />
            <Tooltip 
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={analytics.topPlayers} layout="horizontal">
            ...
            <YAxis dataKey="player_name" type="category" width={100} stroke="#6b7280" />
            <Tooltip 
              contentStyle={{ backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px' }}
            />
            <Legend />
            <Bar dataKey="events" name={t('analytics.totalEvents', 'Total')} fill={COLORS.primary} radius={[0, 4, 4, 0]} />
            <Bar dataKey="passes" name={t('analytics.passes', 'Passes')} fill={COLORS.secondary} radius={[0, 4, 4, 0]} />
            <Bar dataKey="shots" name={t('analytics.shots', 'Shots')} fill={COLORS.accent} radius={[0, 4, 4, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="bg-gradient-to-r from-purple-500 to-blue-500 rounded-lg shadow-lg p-6 text-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <div className="text-sm opacity-90">{match.home_team.name} {t('analytics.events', 'Events')}</div>
          </div>
          <div>
            <div className="text-3xl font-bold">{analytics.awayTotal}</div>
            <div className="text-sm opacity-90">{match.away_team.name} {t('analytics.events', 'Events')}</div>
          </div>
          <div>
            <div className="text-3xl font-bold">{analytics.eventTypes[0]?.type || 'N/A'}</div>
            <div className="text-sm opacity-90">{t('analytics.mostCommon', 'Most Common Event')}</div>
          </div>
          <div>
            <div className="text-3xl font-bold">{analytics.topPlayers[0]?.player_name || 'N/A'}</div>
            <div className="text-sm opacity-90">{t('analytics.mvp', 'Most Active Player')}</div>
          </div>
        </div>
      </div>
    </div>
