...
import { useMatchLogStore, MatchEvent, PendingAckEntry } from '../store/useMatchLogStore';
...
export const useMatchSocket = ({
  ...
  const queuedEventsRef = useRef<Record<string, MatchEvent[]>>(queuedEventsByMatch);
    ...
    (ackResult: AckResultPayload) => {
      ...
      if (!pendingId) {
        if (status === 'success') {
          ...
          console.warn('Ack received with no pending event in queue', ackResult);
        }
        return;
      }
      const entry =
      ...
      if (status === 'success') {
        ...
        console.info('⚠ Duplicate event acknowledged, removed optimistic entry');
      } else {
        if (source === 'live') {
  ...
  const syncQueue = useCallback(() => {
    ...
    console.log(`Syncing ${scopedQueue.length} queued events for match ${matchId}...`);
  ...
  const undoEvent = useCallback(
    (event: MatchEvent) => {
      ...
      const alreadyPending = Boolean(
        pendingAcksRef.current?.[event.client_id]
      );
      if (!alreadyPending) {
  ...
  }, [enabled, matchId, setConnected, syncQueue, flushPendingAcksToQueue, handleAckResult, upsertLiveEvent, getAuthToken]);
