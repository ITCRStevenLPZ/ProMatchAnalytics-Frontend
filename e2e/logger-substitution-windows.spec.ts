...
const resetMatch = async (matchId: string) => {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    console.warn("[logger-substitution] reset failed", response.status());
    try {
      await backendRequest.get("/health");
    } catch (err) {
      console.warn("[logger-substitution] health probe failed", err);
    }
...
test.describe("Logger substitution guardrails", () => {
  ...
  test("blocks invalid substitution and disables confirm", async ({ page }) => {
    ...
    await expect(
      subModal.getByText("Maximum substitution windows reached"),
    ).toBeVisible({ timeout: 10000 });
    await expect(subModal.getByTestId("confirm-substitution")).toBeDisabled();
  });

  test("sends concussion flag and updates concussion count", async ({
    page,
  }) => {
    test.setTimeout(120000);
    ...
    const concussionCard = subModal
      ...
      .first();
    await expect
      .poll(async () => (await concussionCard.textContent()) ?? "")
      .toContain("1");
    await expect(subModal.getByTestId("confirm-substitution")).toBeEnabled();
  });
});
