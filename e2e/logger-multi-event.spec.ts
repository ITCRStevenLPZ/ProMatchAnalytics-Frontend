...
test.beforeEach(async () => {
  const response = await backendRequest.post('/e2e/reset', { data: { matchId: MULTI_MATCH_ID } });
...
test.describe('Logger multi-event timeline', () => {
  ...
  test('records events for both teams and persists after reload', async ({ page }) => {
    ...
    await expectLiveEventCount(page, 2);
    await expect(getLiveEvents(page).filter({ hasText: 'Shot' })).toHaveCount(1);
    await expect(getLiveEvents(page).filter({ hasText: 'Pass' })).toHaveCount(1);
  });
});
