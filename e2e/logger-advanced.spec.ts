...
const promoteToAdmin = async (page: Page) => {
  await page.evaluate(() => {
    ...
    store?.getState?.().setUser?.({ ...currentUser, role: 'admin', displayName: currentUser.displayName || 'E2E Admin' });
  });
};
...
test.describe('Logger substitutions', () => {
  ...
  test('runs substitution wizard and logs substitution event', async ({ page }) => {
  ...
  await confirmBtn.click();
    ...
    await expect(page.getByTestId('live-event-item').first()).toContainText(/Substitution/i);
  });
});
...
test.describe('Logger turbo mode & audio', () => {
  ...
  test('logs turbo pass with recipient and records audio trigger', async ({ page }) => {
    test.setTimeout(120000);
    ...
    await page.addInitScript(() => {
      class StubOscillator {
        ...
        constructor(ctx: any) { this.context = ctx; this.type = 'sine'; this.frequency = { setValueAtTime: () => {} }; }
        connect() {}
        start() { (window as any).__AUDIO_EVENTS__ = ((window as any).__AUDIO_EVENTS__ || 0) + 1; }
        stop() {}
      }
      class StubGain { connect() {}; gain = { setValueAtTime: () => {}, exponentialRampToValueAtTime: () => {} }; }
      class StubAudioContext {
        state = 'running';
        destination = {};
        createOscillator() { return new StubOscillator(this); }
  ...
  await promoteToAdmin(page);
    ...
    await expect(turboPanel.getByText(/Pass needs a recipient/i).first()).toBeVisible();
    ...
    const audioCount = await page.evaluate(() => (window as any).__AUDIO_EVENTS__);
...
test.describe('Logger timers, transitions, and lock state', () => {
  ...
  test('walks halftime/fulltime transitions and enforces lock', async ({ page }) => {
    test.setTimeout(120000);
