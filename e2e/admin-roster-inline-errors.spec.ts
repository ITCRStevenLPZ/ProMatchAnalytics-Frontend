...
test.describe("Admin roster inline error surfacing", () => {
  ...
  test("shows inline error banner + field message when jersey conflicts occur", async ({
    page,
  }) => {
    page.on("console", (msg) => {
    ...
    try {
      ...
      const rosterForm = page
        ...
        .filter({
          has: page.getByRole("button", {
            name: /Add to Roster|Agregar al Plantel/i,
          }),
        })
        .first();
      ...
      console.log(
        "[roster-inline-errors] form text after submit:",
        rosterFormText,
      );
      ...
      await expect(
        page.getByText(
          /Please fix all validation errors before submitting|Por favor, corrija todos los errores de validación antes de enviar/i,
        ),
      ).toBeVisible();
    } finally {
      if (rosterPlayerId) {
