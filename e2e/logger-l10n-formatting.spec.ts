...
test.beforeEach(async () => {
  const response = await backendRequest.post('/e2e/reset', { data: { matchId: L10N_MATCH_ID } });
  expect(response.ok()).toBeTruthy();
});

test.describe('Logger l10n and formatting', () => {
  test('renders Spanish locale labels and preserves formatting after reload', async ({ page }) => {
    ...
    await sendRawEventThroughHarness(page, {
      ...
      player_id: 'HOME-1',
      type: 'Pass',
      data: { pass_type: 'Standard', outcome: 'Complete', receiver_id: 'HOME-2', receiver_name: 'Jugador Casa 2' },
    });
    ...
    await expect(analyticsPanel).toContainText(/Analítica del partido en vivo|Analítica/i);
    ...
    await page.reload();
    await page.getByTestId('toggle-analytics').click();
    await expect(page.getByTestId('analytics-panel')).toBeVisible({ timeout: 15000 });
