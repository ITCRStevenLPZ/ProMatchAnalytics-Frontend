...
const uniqueId = (prefix: string) => {
  const randomToken = Math.random().toString(36).replace(/[^a-z0-9]/gi, '').slice(2, 8).toUpperCase();
...
async function seedTeamWithRoster(label: string): Promise<SeededTeam> {
  ...
  for (let idx = 0; idx < 11; idx += 1) {
    ...
    if (playerResp.status() !== 201) {
      ...
      throw new Error(`Failed to create match player ${playerId}: ${playerResp.status()} ${body}`);
    }
    ...
    if (rosterResp.status() !== 201) {
      ...
      throw new Error(`Failed to attach player ${playerId} to roster: ${rosterResp.status()} ${body}`);
    }
...
test.describe('Admin matches CRUD & status transitions', () => {
  ...
  test('creates matches, walks status transitions, and exposes stats', async () => {
    ...
    await createCompetition(competitionId, `E2E Competition ${competitionId.slice(-4)}`);
    ...
    const createResponse = await apiRequest.post('matches/', { data: matchPayload });
    if (![200, 201].includes(createResponse.status())) {
      const body = await createResponse.text();
      throw new Error(`Failed to create match ${matchPayload.match_id}: ${createResponse.status()} ${body}`);
    }
    ...
    const pendingListResponse = await apiRequest.get('matches/?status=Pending&limit=10');
    const pendingMatches = await json<any[]>(pendingListResponse);
    expect(pendingMatches.some((match) => match.match_id === matchPayload.match_id)).toBeTruthy();

    const invalidTransition = await apiRequest.patch(`matches/${matchDocId}/status`, {
    ...
    const deletableResp = await apiRequest.post('matches/', { data: deletablePayload });
    expect([200, 201]).toContain(deletableResp.status());
    const deletableMatch = await json<any>(deletableResp);

    const deleteResponse = await apiRequest.delete(`matches/${deletableMatch._id}`);
