...
const MAX_ACTIVE_PLAYERS = Number(process.env.PROMATCH_MAX_ACTIVE_PLAYERS ?? 25);
...
async function fetchConflicts(params: {
  ...
  return json<{ conflicts: any[]; total: number; page: number; page_size: number }>(response);
}
...
async function seedPlayerConflictBatch() {
  ...
  const conflictItems = await json<{ items: any[]; total: number }>(conflictItemsResp);
...
test.describe('Admin model CRUD APIs', () => {
  test('Competition CRUD flow', async () => {
    ...
    const createResponse = await apiRequest.post('competitions/', { data: payload });
    ...
    const updateResponse = await apiRequest.put(`competitions/${competitionId}`, {
    ...
    expect(list.items.some((item) => item.competition_id === competitionId)).toBeTruthy();

    const deleteResponse = await apiRequest.delete(`competitions/${competitionId}`);
  ...
  test('Referee CRUD flow', async () => {
    ...
    const createResponse = await apiRequest.post('referees/', { data: payload });
    const created = await json<any>(createResponse);
    ...
    const list = await json<{ items: any[] }>(listResponse);
    expect(list.items.some((item) => item.referee_id === refereeId)).toBeTruthy();
  ...
  test('Team roster enforces unique jersey numbers', async () => {
    ...
    const createTeamResponse = await apiRequest.post('teams/', { data: teamPayload });
    ...
    const duplicateJerseyResponse = await apiRequest.post(`teams/${teamId}/players`, {
      data: {
        team_id: teamId,
    ...
    const rosterResponse = await apiRequest.get(`teams/${teamId}/players?page=1&page_size=5`);
    const roster = await json<{ items: any[] }>(rosterResponse);
    expect(roster.items.filter((item) => item.player_id === playerOneId)).toHaveLength(1);
    expect(roster.items.some((item) => item.player_id === playerTwoId)).toBeFalsy();
  ...
  test('Team roster enforces max active players limit', async () => {
    ...
    const createTeamResp = await apiRequest.post('teams/', { data: teamPayload });
    ...
    await Promise.all(
      createdPlayerIds.map((playerId) => apiRequest.delete(`players/${playerId}`)),
    );
  });
...
test.describe('Ingestion workflow', () => {
  test('processes a team ingestion batch end-to-end', async () => {
    ...
    const itemsResponse = await apiRequest.get(`ingestions/${ingestionId}/items?page=1&page_size=5`);
    ...
    expect(list.batches.some((batchItem) => batchItem.ingestion_id === ingestionId)).toBeTruthy();

    if (finalStatus === 'success') {
      const fetchResponse = await apiRequest.get(`teams/${ingestionTeamId}`);
      expect(fetchResponse.status()).toBe(200);

      const cleanupResponse = await apiRequest.delete(`teams/${ingestionTeamId}`);
  ...
  test('supports ingestion items pagination and filtering', async () => {
    ...
    const firstPage = await json<{ items: any[]; total: number }>(firstPageResp);
  ...
  test('resolves ingestion conflicts via accept and reject flows', async () => {
    const { ingestionId, conflictItems, basePlayerIds } = await seedPlayerConflictBatch();
    ...
    const remainingConflicts = await json<{ total: number }>(remainingConflictsResp);
  ...
  test('lists conflicts globally using filters', async () => {
    ...
    const collectConflicts = async (params?: { target_model?: string; status?: string }) => {
    ...
    const openConflicts = await collectConflicts({ target_model: 'players', status: 'open' });
    ...
    const acceptedConflicts = await collectConflicts({ status: 'closed_accepted' });
    expect(acceptedConflicts).toHaveLength(1);

    const rejectedConflicts = await collectConflicts({ status: 'closed_rejected' });
  ...
  test('retries failed ingestion items after payload fix', async () => {
    ...
    const retryResp = await apiRequest.post(`ingestions/${ingestionId}/retry-failed`, {});
    ...
    const acceptedItems = await json<{ items: any[]; total: number }>(acceptedResp);
