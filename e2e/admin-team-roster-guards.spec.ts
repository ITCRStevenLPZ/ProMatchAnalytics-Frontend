...
test.describe("Admin team roster guardrails", () => {
  ...
  test("team suggestions, roster toggles, duplicate guards, and deletion constraints", async () => {
    ...
    const suggestions =
      await apiJson<Array<{ team_id: string }>>(suggestionsResponse);
    expect(
      suggestions.some((team) => team.team_id === primaryTeam.team_id),
    ).toBeTruthy();
    ...
    const inactiveRoster = await apiJson<{
      items: Array<{ player_id: string }>;
    }>(inactiveRosterResponse);
    expect(
      inactiveRoster.items.some(
        (entry) => entry.player_id === toggledPlayer.player_id,
      ),
    ).toBeTruthy();
    ...
    const activeRoster = await apiJson<{ items: Array<{ player_id: string }> }>(
      activeRosterResponse,
    );
    expect(
      activeRoster.items.some(
        (entry) => entry.player_id === toggledPlayer.player_id,
      ),
    ).toBeFalsy();
    ...
    const duplicateAddResponse = await api.post(
      `teams/${primaryTeam.team_id}/players`,
      {
        data: {
    ...
    const guardedDeleteResponse = await api.delete(
      `admin/teams/${primaryTeam.team_id}`,
    );
    ...
    const removeRoster = async (
      teamId: string,
      roster: typeof primaryTeam.roster,
    ) => {
      await Promise.all(
        roster.map((entry) =>
    ...
    const deletePrimaryResponse = await api.delete(
      `admin/teams/${primaryTeam.team_id}`,
    );
    expect(deletePrimaryResponse.status()).toBe(200);
    const deleteOpponentResponse = await api.delete(
      `admin/teams/${opponentTeam.team_id}`,
    );
