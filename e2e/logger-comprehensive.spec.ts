...
test.describe('Comprehensive Match Logger', () => {
  ...
  test.beforeAll(async ({ playwright }) => {
    ...
    const rosterToMatchLineup = (roster) => roster.map(p => ({ ...p, is_starter: true }));
  ...
  test('logs comprehensive soccer events and tracks effective time', async ({ page }) => {
    test.setTimeout(120000);
    page.on('console', msg => console.log(`BROWSER LOG: ${msg.text()}`));
    page.on('pageerror', err => console.log(`BROWSER ERROR: ${err}`));

    const responsePromise = page.waitForResponse(resp => resp.url().includes('/matches/') && resp.status() === 200);
    ...
    await page.getByTestId(`player-card-${firstPlayerId}`).click({ force: true });
    ...
    const effectiveTimeDisplay = page.locator('text=00:00.000'); // Second clock is effective time
    ...
    const timeText = await page.getByTestId('effective-clock-value').innerText();
    ...
    if (await duplicateBanner.isVisible()) {
        await duplicateBanner.getByText('Dismiss').click();
    }
