...
test.describe('Admin deletion guardrails', () => {
  ...
  test('blocks deleting players/teams with dependencies until cleanup', async () => {
    ...
    const guardedPlayerDelete = await api.delete(`admin/players/${rosterPlayer.player_id}`);
    ...
    await cleanupResource(api, `teams/${guardTeam.team_id}/players/${rosterPlayer.player_id}`);
    const deletePlayerResponse = await api.delete(`admin/players/${rosterPlayer.player_id}`);
    ...
    const guardedTeamDelete = await api.delete(`admin/teams/${rosterHeavyTeam.team_id}`);
    ...
    await Promise.all(
      rosterHeavyTeam.roster.map((entry) =>
        cleanupResource(api, `teams/${rosterHeavyTeam.team_id}/players/${entry.player_id}`),
      ),
    );

    const deleteTeamResponse = await api.delete(`admin/teams/${rosterHeavyTeam.team_id}`);
