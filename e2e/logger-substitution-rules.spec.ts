...
test.describe('Logger substitution rules', () => {
  ...
  test('allows extra-time window and shows updated counters', async ({ page }) => {
    ...
    await expect(subModal.getByText(/Substitutions|Sustituciones/i).locator('..')).toContainText('1/6');
    await expect(subModal.getByText(/Windows|Ventanas/i).locator('..')).toContainText('1/4');
  ...
  test('blocks re-entry of substituted-out player', async ({ page }) => {
    ...
    await expect(subModal.getByText(/Invalid Substitution|Sustituci[oó]n inv[aá]lida/i)).toBeVisible({ timeout: 10000 });
    await expect(subModal.getByText(/already substituted out|ya fue sustituido/i)).toBeVisible();
