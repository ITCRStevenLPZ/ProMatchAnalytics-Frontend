...
test.describe("Admin create-match lineup defaults", () => {
  ...
  test("disables substitute checkbox when the same player is selected as starter", async ({
    page,
  }) => {
    const suffix = uniqueId("MBLOCK");
    const competitionId = uniqueId("COMP");
    const venueId = uniqueId("VEN");
    const refereeId = uniqueId("REF");
    ...
    const homeTeam = await seedTeam(api, {
      team_id: uniqueId("TEAMH"),
      name: `Block Home ${suffix}`,
    });
    const awayTeam = await seedTeam(api, {
      team_id: uniqueId("TEAMA"),
      name: `Block Away ${suffix}`,
    });
    ...
    try {
      ...
      const homeStarterFirst = page
        .getByTestId("home-starters-candidates")
        .locator("input[type='checkbox']")
        .first();
      const homeSubFirst = page
        .getByTestId("home-substitutes-candidates")
        .locator("input[type='checkbox']")
        .first();

      await expect(homeSubFirst).toBeEnabled();
      await homeStarterFirst.check();
      await expect(homeSubFirst).toBeDisabled();
      ...
      const awayStarterFirst = page
        .getByTestId("away-starters-candidates")
        .locator("input[type='checkbox']")
        .first();
      const awaySubFirst = page
        .getByTestId("away-substitutes-candidates")
        .locator("input[type='checkbox']")
        .first();

      await expect(awaySubFirst).toBeEnabled();
      await awayStarterFirst.check();
      await expect(awaySubFirst).toBeDisabled();
    } finally {
      for (const entry of homeTeam.roster) {
        await cleanupResource(api, `players/${entry.player_id}`).catch(
          () => {},
