...
test.describe("Admin duplicate review flows", () => {
  ...
  test("duplicate warning inline action opens the SimilarRecordsViewer", async ({
    page,
  }) => {
    ...
    try {
      ...
      const duplicateBanner = page.getByText(
        /(Potential Duplicates Found|Posibles Duplicados Encontrados)/i,
      );
      await expect(duplicateBanner).toBeVisible();

      const reviewButton = page.getByRole("button", {
        name: /(Review similar records|Revisar registros similares)/i,
      });
      await expect(reviewButton).toBeVisible();
      await reviewButton.click();

      const viewerHeading = page.getByRole("heading", {
        name: /(Similar Records Found|Registros Similares Encontrados)/i,
      });
