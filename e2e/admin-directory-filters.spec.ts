...
const buildPlayerPayload = (overrides: Partial<Record<string, any>> = {}) => ({
  ...
  name:
    overrides.name ?? `E2E Player ${Math.random().toString(16).slice(2, 6)}`,
  nickname: overrides.nickname ?? "FilterTest",
  birth_date:
    overrides.birth_date ?? new Date("1995-02-15T00:00:00Z").toISOString(),
...
async function cleanupFilterTestPlayers(): Promise<void> {
  ...
  try {
    const response = await apiRequest.get(
      `players/?${searchParams.toString()}`,
    );
    if (!response.ok()) {
      console.warn(
        "[admin-directory-filters] Failed to fetch stale players:",
        await response.text(),
      );
      return;
    }
    ...
    await Promise.all(
      targets.map(async (player) => {
        const deleteResp = await apiRequest.delete(
...
test.describe("Admin directory filters & suggestions", () => {
  ...
  test("filters players by position, country, and age; returns search suggestions", async () => {
    ...
    const cmResponse = await apiRequest.get(
      "players/?page=1&page_size=10&position=CM",
    );
    const cmList = await json<{ items: any[]; total: number }>(cmResponse);
    const cmIds = cmList.items.map((item) => item.player_id);
    expect(cmIds).toEqual(
      expect.arrayContaining([
        playerPayloads[0].player_id,
        playerPayloads[2].player_id,
      ]),
    );
    ...
    const usaCmResponse = await apiRequest.get(
      `players/?${usaCmQuery.toString()}`,
    );
    const usaCmList = await json<{ items: any[]; total: number }>(
      usaCmResponse,
    );
    ...
    const ageFilteredResponse = await apiRequest.get(
      `players/?${ageFilteredQuery.toString()}`,
    );
    const ageFiltered = await json<{ items: any[] }>(ageFilteredResponse);
    const ageFilteredIds = ageFiltered.items.map((item) => item.player_id);
    expect(ageFilteredIds).toEqual(
      expect.arrayContaining([playerPayloads[0].player_id]),
    );
    expect(ageFilteredIds).not.toContain(playerPayloads[2].player_id);

    const suggestionResponse = await apiRequest.get(
      "players/search/suggestions?q=Filter Target USA",
    );
    const suggestions = await json<any[]>(suggestionResponse);
    expect(
      suggestions.some(
        (player) => player.player_id === playerPayloads[0].player_id,
      ),
    ).toBeTruthy();

    await Promise.all(
      cleanupIds.map((playerId) => apiRequest.delete(`players/${playerId}`)),
    );
  });
  ...
  test("team search suggestions and roster filters honor is_active/position flags", async () => {
    ...
    const keeper = buildPlayerPayload({
      player_id: uniqueId("GK"),
      position: "GK",
      name: "Directory Keeper",
    });
    const defender = buildPlayerPayload({
      player_id: uniqueId("CB"),
      position: "CB",
      name: "Directory Defender",
    });
    ...
    if (addKeeper.status() !== 201) {
      ...
      throw new Error(
        `Failed to add keeper to roster: ${addKeeper.status()} ${body}`,
      );
    }
    ...
    if (addDefender.status() !== 201) {
      ...
      throw new Error(
        `Failed to add defender to roster: ${addDefender.status()} ${body}`,
      );
    }

    const deactivateDefender = await apiRequest.put(
      `teams/${teamId}/players/${defender.player_id}`,
      {
    ...
    const activeRosterResponse = await apiRequest.get(
      `teams/${teamId}/players?page=1&page_size=10&is_active=true`,
    );
    const activeRoster = await json<{ items: any[] }>(activeRosterResponse);
    expect(
      activeRoster.items.some((item) => item.player_id === keeper.player_id),
    ).toBeTruthy();
    expect(
      activeRoster.items.some((item) => item.player_id === defender.player_id),
    ).toBeFalsy();

    const goalieRosterResponse = await apiRequest.get(
      `teams/${teamId}/players?page=1&page_size=10&position=GK`,
    );
    ...
    const suggestionResponse = await apiRequest.get(
      `teams/search/suggestions?q=${suggestionQuery}`,
    );
    const teamSuggestions = await json<any[]>(suggestionResponse);
    expect(
      teamSuggestions.some((team) => team.team_id === teamId),
    ).toBeTruthy();
